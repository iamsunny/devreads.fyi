...
// Feed fields with XML attributes parse as objects ({ _: 'text', $: {...} })
// rather than strings; coerce before any string handling.
...
export function postKey(item) {
  const link = item.link ? canonicalUrl(asText(item.link)) : '';
  const guid = asText(item.guid).trim();
  if (guid) return hash(guid);
  if (link) return hash(link);
  return hash(`${asText(item.title)}|${item.isoDate ?? item.pubDate ?? ''}`);
}
...
// Tracking pixels and stat beacons that show up as the "first image" in feeds.
const IMAGE_BLOCKLIST = /(medium\.com\/_\/stat|feedburner|doubleclick|\/pixel)/i;

function extractImage(item) {
  const candidates = [];
  for (const m of item.mediaThumbnail ?? []) candidates.push(m?.$?.url ?? m?.url);
  for (const m of item.mediaContent ?? []) {
    const attrs = m?.$ ?? {};
    if (attrs.medium === 'image' || /^image\//.test(attrs.type ?? '') || /\.(jpe?g|png|webp|gif)(\?|$)/i.test(attrs.url ?? '')) {
      candidates.push(attrs.url);
    }
  }
  if (/^image\//.test(item.enclosure?.type ?? '')) candidates.push(item.enclosure.url);
  const html = asText(item.contentEncoded ?? item.content);
  const img = html.match(/<img[^>]+src=["']([^"']+)["']/i);
  if (img) candidates.push(img[1]);

  for (const url of candidates) {
    if (typeof url === 'string' && url.startsWith('https://') && !IMAGE_BLOCKLIST.test(url)) return url;
  }
  return null;
}
...
export function normalizeItem(item) {
  ...
  return {
    ...(image ? { image } : {}),
    id: postKey(item),
    url: item.link ? canonicalUrl(asText(item.link)) : null,
