...
let current;
try {
  current = parse(readFileSync('sites.yml', 'utf8'))?.sites;
} catch (err) {
  console.error(`sites.yml is not valid YAML: ${err?.message ?? err}`);
  process.exit(1);
}
...
for (const s of current) {
  ...
  for (const field of ['id', 'name', 'feed', 'url']) {
    if (typeof s?.[field] !== 'string' || !s[field].trim()) fail(`${label}: missing or empty "${field}"`);
  }
  if (s?.id && !/^[a-z0-9][a-z0-9-]*$/.test(s.id))
    fail(`${label}: id must be kebab-case (lowercase letters, digits, hyphens) - it becomes a URL and a filename`);
...
if (errors.length) {
  ...
  process.exitCode = 1; // let Node drain open sockets instead of hard-exiting
} else {
